...
```
wss://api.{environment}.corti.app/v2/
```
...
## Endpoint: `/interactions/{id}/stream`

### Establishing a Connection

Clients must initiate a WebSocket connection using the `wss://` scheme and provide an interaction ID in the URL.
When creating an interaction, the 200 response provides the URL for that interaction including the Tenant-Name URL parameter.

<Note>The authentication for the WSS stream requires in addition to the `Tenant-Name` parameter a `token` parameter to pass in the Bearer access token.</Note>

### Request

```
GET /interactions/{id}/stream
```
