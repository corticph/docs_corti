---
title: Stream - Ambient
description: WebSocket Secure (WSS) API Documentation for /stream
tag: WSS
---
...
<Info>By default, returned data streams as well as incoming audio are being saved. Based on the interactionId you can find the saved `transcripts` and `facts` and `recording(s)` in the relevant REST endpoints.
